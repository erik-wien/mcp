...
A virtualenv is not optional here: the Homebrew Python on the work machines is
PEP 668 "externally managed", so a global `pip install pytest` is refused. The
dependency files were always correct — what was missing until 2026-09-04 was
the environment itself, which is why `mcp/tests/` had gone unrun long enough
for nobody to notice.

`.venv/` is gitignored. `requirements-dev.txt` pulls in `requirements.txt`, so
the venv gets `pyyaml` too — it does **not** inherit system site-packages, and
without it the first run fails with `ModuleNotFoundError` instead of a test
result.
